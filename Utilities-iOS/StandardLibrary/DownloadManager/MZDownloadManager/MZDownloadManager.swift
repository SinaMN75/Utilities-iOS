...
extension MZDownloadManager {
    ...
    fileprivate func populateOtherDownloadTasks() {
        ...
        for downloadTask in downloadTasks {
            ...
            let fileId = Int(taskDescComponents[TaskDescFileDestinationIndex]) ?? 0
            
            let downloadModel = MZDownloadModel.init(fileName: fileName, fileURL: fileURL, destinationPath: destinationPath, fileId: fileId)
...
extension MZDownloadManager: URLSessionDownloadDelegate {
        ...
        for (index, downloadModel) in downloadingArray.enumerated() {
            if downloadTask.isEqual(downloadModel.task) {
                ...
                if fileManager.fileExists(atPath: basePath) {
                    ...
                    do { try fileManager.moveItem(at: location, to: fileURL) }
                    ...
                    if let _ = self.delegate?.downloadRequestDestinationDoestNotExists {
                        ...
                        self.delegate?.downloadRequestDidFailedWithError?(NSError(domain: "FolderDoesNotExist",
                                                                                  code: 404,
                                                                                  userInfo: [NSLocalizedDescriptionKey : "Destination folder does not exists"]),
                                                                          downloadModel: downloadModel, index: index)
                    }
                }
                break
            }
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        DispatchQueue.main.async {
            ...
            if (err?.userInfo[NSURLErrorBackgroundTaskCancelledReasonKey] as? NSNumber)?.intValue == NSURLErrorCancelledReasonUserForceQuitApplication || (err?.userInfo[NSURLErrorBackgroundTaskCancelledReasonKey] as? NSNumber)?.intValue == NSURLErrorCancelledReasonBackgroundUpdatesDisabled {
                ...
                let fileId = Int(taskDescComponents[self.TaskDescFileDestinationIndex]) ?? 0
                let downloadModel = MZDownloadModel.init(fileName: fileName, fileURL: fileURL, destinationPath: destinationPath, fileId: fileId)
                downloadModel.status = TaskStatus.failed.description()
                downloadModel.task = downloadTask
                let resumeData = err?.userInfo[NSURLSessionDownloadTaskResumeData] as? Data
                var newTask = downloadTask
                if self.isValidResumeData(resumeData) == true { newTask = self.sessionManager.downloadTask(withResumeData: resumeData!) }
                else { newTask = self.sessionManager.downloadTask(with: URL(string: fileURL as String)!) }
                ...
                for(index, object) in self.downloadingArray.enumerated() {
                    ...
                    if task.isEqual(downloadModel.task) {
                        if err?.code == NSURLErrorCancelled || err == nil {
                            ...
                            if err == nil { self.delegate?.downloadRequestFinished?(downloadModel, index: index) }
                            else { self.delegate?.downloadRequestCanceled?(downloadModel, index: index) }
                        } else {
                            let resumeData = err?.userInfo[NSURLSessionDownloadTaskResumeData] as? Data
                            var newTask = task
                            if self.isValidResumeData(resumeData) == true { newTask = self.sessionManager.downloadTask(withResumeData: resumeData!) }
                            else { newTask = self.sessionManager.downloadTask(with: URL(string: downloadModel.fileURL)!) }
                            newTask.taskDescription = task.taskDescription
                            downloadModel.status = TaskStatus.failed.description()
                            downloadModel.task = newTask as? URLSessionDownloadTask
                            self.downloadingArray[index] = downloadModel
                            if let error = err { self.delegate?.downloadRequestDidFailedWithError?(error, downloadModel: downloadModel, index: index) }
                            else {
                                self.delegate?.downloadRequestDidFailedWithError?(NSError(domain: "MZDownloadManagerDomain",
                                                                                          code: 1000,
    ...
    public func urlSessionDidFinishEvents(forBackgroundURLSession session: URLSession) {
        if let backgroundCompletion = self.backgroundSessionCompletionHandler { DispatchQueue.main.async(execute: { backgroundCompletion() }) }
...
extension MZDownloadManager {
    ...
    @objc public func addDownloadTask(_ fileName: String, request: URLRequest, destinationPath: String, fileId: Int) {
    ...
    @objc public func addDownloadTask(_ fileName: String, fileURL: String, destinationPath: String, fileId: Int) {
        
        let url = URL(string: fileURL)!
        let request = URLRequest(url: url)
        addDownloadTask(fileName, request: request, destinationPath: destinationPath, fileId: fileId)
        
    }
    
    @objc public func addDownloadTask(_ fileName: String, fileURL: String, fileId: Int) {
        addDownloadTask(fileName, fileURL: fileURL, destinationPath: "", fileId: fileId)
    }
    
    @objc public func addDownloadTask(_ fileName: String, request: URLRequest, fileId: Int) {
        addDownloadTask(fileName, request: request, destinationPath: "", fileId: fileId)
    }
    ...
    @objc public func cancelTaskById(_ id: Int) {
        let item = downloadingArray.filter { item in
            if item.fileId == id { return true }
            else { return false }
        }
        
        item.first?.task?.cancel()
    }
