...
@objc public class Subtitles : NSObject {
    ...
    fileprivate static func parseSubRip(_ payload: String) -> NSDictionary? {
        do {
            ...
            let matches = regex.matches(in: payload,
            ...
            for m in matches {
                ...
                var match = regex.matches(in: group,
                                          options: NSRegularExpression.MatchingOptions(rawValue: 0),
                                          range: NSMakeRange(0, group.count))
                guard let i = match.first else { continue }
                let index = (group as NSString).substring(with: i.range)
                
                // Get "from" & "to" time
                regex = try NSRegularExpression(pattern: "\\d{1,2}:\\d{1,2}:\\d{1,2}[,.]\\d{1,3}", options: .caseInsensitive)
                match = regex.matches(in: group,
                                      options: NSRegularExpression.MatchingOptions(rawValue: 0),
                                      range: NSMakeRange(0, group.count))
                guard match.count == 2 else { continue }
                guard let from = match.first, let to = match.last else { continue }
                ...
                guard (group as NSString).length - range.length > 0 else { continue }
    ...
    fileprivate static func searchSubtitles(_ payload: NSDictionary?, _ time: TimeInterval) -> String? {
        ...
        guard let values = payload?.allValues, let result = (values as NSArray).filtered(using: predicate).first as? NSDictionary else { return nil }
        guard let text = result.value(forKey: "text") as? String else { return nil }
...
public extension AVPlayerViewController {
    ...
    fileprivate var parsedPayload: NSDictionary? {
        ...
        set (value) { objc_setAssociatedObject(self, &AssociatedKeys.PayloadKey, value,
    ...
    fileprivate func addSubtitleLabel() {
        guard let _ = subtitleLabel else {
            ...
            // Position
            var constraints = NSLayoutConstraint.constraints(withVisualFormat: "H:|-(20)-[l]-(20)-|",
                                                             options: NSLayoutConstraint.FormatOptions(rawValue: 0),
                                                             metrics: nil,
                                                             views: ["l" : subtitleLabel!])
            ...
            constraints = NSLayoutConstraint.constraints(withVisualFormat: "V:[l]-(30)-|",
                                                         ...
                                                         views: ["l" : subtitleLabel!])
            contentOverlayView?.addConstraints(constraints)
            subtitleLabelHeightConstraint = NSLayoutConstraint(item: subtitleLabel!,
                                                               attribute: .height,
                                                               relatedBy: .equal,
                                                               toItem: nil,
